package io.github.crucible.omniconfig.packets;
...
public class PacketSyncOptions {
    ...
    /*
	public static void encode(PacketSyncOptions msg, ByteBuf buf) {
    ...
    if (wrapper != null) {
        ...
        OmniconfigCore.logger.info("Synchronizing values of " + msg.fileName + " with ones dispatched by server...");

        for (String id : msg.synchronizedParameters.keySet()) {
            Omniconfig.GenericParameter parameter = this.wrapper.invokationMap.get(id);
            ...
            if (parameter != null) {
                ...
                OmniconfigCore.logger.error("Value '" + id + "' does not exist in " + msg.fileName + "! Skipping.");
            }
        }
    } else {
        OmniconfigCore.logger.fatal("Received synchronization packet for non-existent config file: " + msg.fileName);
    }
